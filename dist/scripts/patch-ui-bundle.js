...
if (CHECK_MODE) {
    const checks = [
        ...
        },
        {
            ...
            test: () => /\w+==="opencode_local"\|\|\w+==="kilo_local"/.test(content),
        },
        {
...
patch("1b. Upload adapter Set", /new Set\(\["claude_local","codex_local","opencode_local"\]\)/, 'new Set(["claude_local","codex_local","opencode_local","kilo_local"])');
...
if (localAdapterSetPattern.test(content)) {
    ...
    content = content.replace(localAdapterSetPattern, 'new Set(["claude_local","codex_local","gemini_local","opencode_local","kilo_local","pi_local","cursor","hermes_local"])');
    patchCount++;
...
content = content.replace(displayNamePattern, 'opencode_local:"OpenCode (local)",kilo_local:"Kilo (local)"');
...
patch("1e. Kilo adapter definition", /(\w+)=\{type:"opencode_local",label:"OpenCode \(local\)",(parseStdoutLine:\w+,ConfigFields:\w+,buildAdapterConfig:\w+)\}/, (_match, varName, sharedFields) => `${_match},Z9Kilo={type:"kilo_local",label:"Kilo (local)",${sharedFields}}`);
// 1f. Register Z9Kilo in the adapter array — add before hermes adapter entry
patch("1f. Register Z9Kilo in adapter array", /,([\w]+)=\{type:"hermes_local"/, (_match, hermesVar) => `,Z9Kilo,${hermesVar}={type:"hermes_local"`);
// 1g. Dropdown option entries — add kilo option after opencode option
patch("1g. Dropdown option entry", /(\{value:"opencode_local",label:"OpenCode[^"]*"[^}]+\})/g, (match) => {
...
patch("1h. Thinking effort options", /(opencode_local:(\[\{value:""[^\]]+\]))/, (_match, fullEntry, optionsArray) => `${fullEntry},kilo_local:${optionsArray}`);
// ============================================================
...
patch("2a. Options label ternary", /(\w+)==="opencode_local"\?"OpenCode options":"Agent options"/, (_match, varName) => `${varName}==="opencode_local"?"OpenCode options":${varName}==="kilo_local"?"Kilo options":"Agent options"`);
// ============================================================
...
let eqCount = 0;
content = content.replace(/(\w+)==="opencode_local"/g, (_match, varName) => {
    eqCount++;
...
console.log(`\n✓ Patched: ${filePath}`);
//# sourceMappingURL=patch-ui-bundle.js.map