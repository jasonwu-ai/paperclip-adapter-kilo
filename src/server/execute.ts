...
import type { ChildProcessResult } from "@paperclipai/adapter-utils/server-utils";
import type {
  AdapterExecutionContext,
  AdapterExecutionResult,
} from "@paperclipai/adapter-utils";
import { parseKiloJsonl, isKiloUnknownSessionError } from "./parse.js";
import type { KiloParsedResult } from "./parse.js";
...
function parseModelProvider(model: string): string | null {
...
interface RunAttemptResult {
  proc: ChildProcessResult;
  rawStderr: string;
  parsed: KiloParsedResult;
}
...
export async function execute(
  ...
  const rtSid = asString(
    rtSp.sessionId,
    (runtime.sessionId as string | undefined) ?? "",
  );
  ...
  if (rtSid && !canResume) {
    await onLog(
      "stdout",
      `[paperclip] Kilo session "${rtSid}" saved for "${rtScwd}", not resuming in "${cwd}".\n`,
    );
  }
  ...
  if (resolvedInstr) {
    try {
      ...
      await onLog(
        "stdout",
        `[paperclip] Warning: could not read instructions "${resolvedInstr}": ${msg}\n`,
      );
    }
  }

  // ---- Prompt --------------------------------------------------------------
  const promptTpl = asString(
  ...
  const bsRendered =
    !sessionId && bsTpl.trim().length > 0
      ? renderTemplate(bsTpl, td).trim()
      : "";
  const handoff = asString(
  ...
  const wComment =
    nonEmptyString(context.wakeCommentId) ||
  ...
  if (wTask) env.PAPERCLIP_TASK_ID = wTask;
  if (wReason) env.PAPERCLIP_WAKE_REASON = wReason;
  if (wComment) env.PAPERCLIP_WAKE_COMMENT_ID = wComment;
  if (wApprId) env.PAPERCLIP_APPROVAL_ID = wApprId;
  if (wApprSt) env.PAPERCLIP_APPROVAL_STATUS = wApprSt;
  if (wLinked.length > 0)
    env.PAPERCLIP_LINKED_ISSUE_IDS = wLinked.join(",");
  if (effectiveWsCwd) env.PAPERCLIP_WORKSPACE_CWD = effectiveWsCwd;
  if (wsSrc) env.PAPERCLIP_WORKSPACE_SOURCE = wsSrc;
  if (wsId) env.PAPERCLIP_WORKSPACE_ID = wsId;
  if (wsRepo) env.PAPERCLIP_WORKSPACE_REPO_URL = wsRepo;
  if (wsRef) env.PAPERCLIP_WORKSPACE_REPO_REF = wsRef;
  if (wsAh) env.AGENT_HOME = wsAh;
  if (wsHints.length > 0)
  ...
  if (!hasExplicitKey && authToken) env.PAPERCLIP_API_KEY = authToken;
  ...
  // ---- Args builder --------------------------------------------------------
  const buildArgs = (sid: string | null): string[] => {
    const a = ["run", "--format", "json", "--auto"];
    if (model) a.push("-m", model);
    if (variant) a.push("--variant", variant);
    if (config.thinking) a.push("--thinking");
    a.push("--dir", cwd);
    if (sid) a.push("--session", sid);
    if (extraArgs.length > 0) a.push(...extraArgs);
    return a;
  };
  ...
  const toResult = (
    ...
    const rSid =
      ...
      (clearSession
        ? null
        : (rtSid || (runtime.sessionId as string | undefined) || null));
    const rSp = rSid
      ? {
          sessionId: rSid,
          cwd,
    ...
    const errMsg =
      pErr || stderrLine || `Kilo exited with code ${exitCode ?? -1}`;
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[paperclip] Kilo session "${sessionId}" unavailable; retrying fresh.\n`,
    );
